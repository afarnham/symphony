defmodule SymphonyElixir.Codex.AppServer do
  ...
  require Logger
  alias SymphonyElixir.{Codex.DynamicTool, Config, Config.Schema, PathSafety, SSH}
  ...
  def start_session(workspace, opts \\ []) do
    ...
    with {:ok, expanded_workspace} <- validate_workspace_cwd(workspace, worker_host, settings),
         {:ok, port} <- start_port(expanded_workspace, worker_host, dynamic_tool_binding, settings) do
  ...
  defp start_port(workspace, nil, dynamic_tool_binding, settings) do
      ...
      port =
        Port.open(
          ...
          [
            ...
            args: [
              ~c"-lc",
              String.to_charlist(local_launch_command(dynamic_tool_binding, settings))
            ],
  ...
  defp start_port(workspace, worker_host, dynamic_tool_binding, settings)
       when is_binary(worker_host) do
    remote_command = remote_launch_command(workspace, dynamic_tool_binding, settings)
    SSH.start_port(worker_host, remote_command, line: @port_line_bytes)
  end

  defp local_launch_command(dynamic_tool_binding, settings) do
    [
  ...
  defp remote_launch_command(workspace, dynamic_tool_binding, settings)
  ...
  defp session_policies(workspace, worker_host, settings) do
    policy_opts = if is_binary(worker_host), do: [remote: true], else: []

    with {:ok, turn_sandbox_policy} <-
           Schema.resolve_runtime_turn_sandbox_policy(settings, workspace, policy_opts) do
      {:ok,
       %{
