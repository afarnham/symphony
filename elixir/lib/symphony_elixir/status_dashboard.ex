defmodule SymphonyElixir.StatusDashboard do
  ...
  defp snapshot_with_samples(token_samples, now_ms) do
    case snapshot_payload() do
      {:ok, %{running: running, retrying: retrying, codex_totals: codex_totals} = snapshot} ->
        agent_totals = Map.get(snapshot, :agent_totals, codex_totals)
  ...
  defp format_running_summary(running_entry, running_event_width) do
    ...
    total_tokens = Map.get(running_entry, :agent_total_tokens) || running_entry.codex_total_tokens || 0
  ...
  defp summarize_message(%{kind: kind, payload: payload}) do
    humanize_agent_event(kind, payload)
  end

  defp summarize_message(nil), do: "no agent message yet"
  defp summarize_message(message), do: humanize_codex_message(message)

  defp humanize_agent_event(:session_started, _payload), do: "session started"
  defp humanize_agent_event(:turn_started, _payload), do: "turn started"
  defp humanize_agent_event(:turn_completed, _payload), do: "turn completed"
  defp humanize_agent_event(:turn_failed, payload), do: "turn failed: #{format_reason(payload)}"
  defp humanize_agent_event(:input_required, _payload), do: "turn blocked: waiting for user input"

  defp humanize_agent_event(:assistant_text, payload),
    do: map_value(payload, [:text, "text"]) || "assistant response"

  defp humanize_agent_event(:usage_updated, payload) do
    usage = map_value(payload, [:usage, "usage"]) || %{}
    total = map_value(usage, [:total_tokens, "total_tokens"])
    if is_integer(total), do: "usage updated: #{format_count(total)} tokens", else: "usage updated"
  end

  defp humanize_agent_event(kind, _payload), do: kind |> to_string() |> String.replace("_", " ")
